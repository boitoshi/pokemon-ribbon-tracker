...
/** リボン/あかし画像の配信ベースURL（ConoHa WING 上の配置規約） */
const ASSET_BASE = 'https://www.pokebros.net/wp-content/uploads/pokemon-assets';
...
/**
 * catalog の key から画像URLを生成する（ブログ側ジェネレーターと同一規約）。
 * - kind==='ribbon': key の `-` を `_` に置換して ribbons/<slug>.png
 * - kind==='mark':   key 末尾の `-mark` を除去してから `-`→`_` で marks/<slug>.png
 */
function toImageUrl(elem) {
	if (elem.kind === 'mark') {
		const slug = elem.key.replace(/-mark$/, '').replace(/-/g, '_');
		return `${ASSET_BASE}/marks/${slug}.png`;
	}
	const slug = elem.key.replace(/-/g, '_');
	return `${ASSET_BASE}/ribbons/${slug}.png`;
}
...
// 検証: image_url がカタログ全体でユニーク（1カタログ要素 = 1画像ファイル。
// 同一要素の複数 route は同じ image_url を共有するため、要素単位で判定する）
const seenImageUrls = new Set();

for (const elem of catalog) {
	const imageUrl = toImageUrl(elem);
	if (seenImageUrls.has(imageUrl)) {
		errors.push(`${elem.key}: image_url が重複しています（${imageUrl}）`);
	}
...
function entryToTs(entry) {
	...
	if (entry.image_url !== undefined) lines.push(`    image_url: ${q(entry.image_url)},`);
