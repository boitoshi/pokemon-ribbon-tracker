...
公開URL: **https://www.pokebros.net/ribbon/**（Apache 共有ホスティングのサブディレクトリ）

`main` ブランチへの push で GitHub Actions（`.github/workflows/deploy.yml`）が
lint / check / test → `BASE_PATH=/ribbon` ビルド → FTP アップロードを自動実行します。
...
| `FTP_SERVER_DIR` | アップロード先（例: `/pokebros.net/ribbon/`、末尾スラッシュ必須） |

### ローカルでの本番ビルド検証

```sh
npm run build:prod   # BASE_PATH=/ribbon でビルド
npx vite preview     # http://localhost:4173/ribbon/ で確認
```
