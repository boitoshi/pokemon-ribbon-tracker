...
export function getRibbonEvaluation(
	...
	// 種族チェック: ポケモン種族の登場世代 > リボンを取得できるソフトの最大世代 → locked
	// （例: Gen9初出の種族でも SV で取れる SwSh 初出あかしは対象。games 不明時はリボン定義世代で判定）
	if (pokemon) {
		const obtainableGens = ribbon.games
			.map((gid) => gameGenMap.get(gid))
			.filter((g): g is number => g !== undefined);
		const maxObtainableGen =
			obtainableGens.length > 0 ? Math.max(...obtainableGens) : ribbon.generation;
		if (pokemon.generation > maxObtainableGen) {
