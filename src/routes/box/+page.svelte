<script lang="ts">
	...
	const mobileSearchResults = $derived(
		(() => {
			...
			return ribbonProgress.allPokemon
				.filter((p) => p.name.includes(query))
				.slice(0, MOBILE_MAX_RESULTS);
		})()
	);
	...
	const completionPercent = $derived(
		countableRibbons.length > 0 ? Math.round((totalObtained / countableRibbons.length) * 100) : 0
	);
	...
	const activePokemonName = $derived(
		ribbonProgress.activeMyPokemon
			? ribbonProgress.activeMyPokemon.nickname ||
					ribbonProgress.allPokemon.find((p) => p.id === ribbonProgress.activeMyPokemon!.pokemonId)
						?.name ||
					''
			: ''
	);
				...
				{#each ribbonProgress.myPokemonList as mp (mp.id)}
					...
					>
						{#if getMyPokemonImage(mp.pokemonId)}
							<img
								src={getMyPokemonImage(mp.pokemonId)}
								alt={getMyPokemonName(mp)}
...
<div class="mx-auto max-w-7xl px-2 py-3 md:px-4 md:py-6">
	...
	<div class="mb-4 flex items-center justify-between gap-2">
		<div>
			<h1 class="text-lg font-bold text-gray-800 md:text-xl">📦 きろく</h1>
	...
	<div class="flex flex-col gap-4 md:flex-row md:gap-6">
		...
		<main class="min-w-0 flex-1">
			{#if hasNoActivePokemon}
				<div class="mb-4 rounded-xl border border-sky-200 bg-sky-50 px-4 py-4">
					...
					<p class="mt-1 text-sm text-sky-800">
						今は<strong>参照モード</strong>です（一覧・フィルタ・可否確認）。 マイポケモンを選ぶと<strong
							>記録モード</strong
						>になり、進捗や取り逃しを保存できます。
					</p>
					<div class="mt-3 flex flex-wrap gap-2">
						<a
							...
							>⚙️ セットアップを始める</a
						>
						<p class="self-center text-xs text-sky-600">
							← 所持ゲーム・ハードを登録するとリボンの取得可否が分かります
						</p>
					</div>
				</div>
			{/if}
			...
			<!-- ヒーローカード -->
			{#if ribbonProgress.activeMyPokemon}
				{@const activeGen = ribbonProgress.genMap.get(
					ribbonProgress.activeMyPokemon?.originGame ?? ''
				)}
						...
						<div class="flex items-center gap-4">
							...
							<div class="min-w-0 flex-1">
								...
								<div class="flex items-center justify-between text-xs text-gray-600">
									<span
										>{totalObtained}個取得 / 残り{countableRibbons.length - totalObtained}個</span
									>
									<span class="font-bold {theme.accent}">{completionPercent}%</span>
								</div>
								{#if missedRibbons.length > 0}
									<p class="text-xs text-red-600 font-medium">
										❌ 取り逃し {missedRibbons.length}個
									</p>
								{/if}
				...
				<div class="mb-3 flex items-center justify-between">
					...
					<div class="flex overflow-hidden rounded-lg border border-gray-200">
						<button
							...
							onclick={() => (viewMode = 'grid')}>⊞ グリッド</button
						>
						<button
							...
							onclick={() => (viewMode = 'list')}>☰ リスト</button
						>
					</div>
				</div>
