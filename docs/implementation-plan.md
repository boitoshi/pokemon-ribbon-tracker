...
### Phase 15: リボン＆あかしコンプ機能

Gen 9 のあかしデータが整備されたことで「全リボン + 全あかし」コンプリートの追跡が可能になる。

#### 背景
- **遭遇あかし**: フィールドで出会った際にランダムで付く（捕獲時確定）
- **外付けあかし**: 捕獲後に特定のアクションで取得可能
  - ソシアルあかし（ピクニックで遊ぶ）
  - くいしんぼのあかし（ピクニックでごはんを食べさせる）
  - ヌシのあかし（ヌシポケモンを捕まえる）
  - さいきょうのあかし（最強レイドで捕まえる）
  - バディのあかし（テイムしてバディにする）
- 外付けあかしはどのポケモンでも後から取得できるためコンプの核心

#### 15a. あかしフィルタ対応
- `RibbonFilter.vue` / `ribbonFilter.ts` に `type: 'mark'` フィルタを追加
- フィルタに「あかし」タブ/バッジを追加

#### 15b. コンプ進捗の分離表示
- 現在の進捗バーをリボン / あかし で分割表示
  ```
  リボン: ██████░░░░ 60/100
  あかし: ████░░░░░░ 20/49
  ```
...
#### 15c. 外付けあかしの優先表示
- `mark` かつ遭遇依存でないあかしを「今すぐ取れる」セクションとして優先表示
- `eligibility` に `obtainable: 'action'` フィールドを追加して識別（型拡張）
...
| **高** | Phase 15（リボン＆あかしコンプ） | 未着手 | あかしデータ整備済み、UI対応が次の一手 |
